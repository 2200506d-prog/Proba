...
/**
 * Renderiza la primera página en canvas
 */
...
/**
 * Extrae texto de todas las páginas (suele bastar para CFE)
 */
...
/**
 * Detecta tarifa (CFE suele traer "Tarifa: 1C", "TARIFA 1", "Tarifa DAC", etc.)
 */
function detectTariff(text) {
  const t = normalizeText(text);

  // Ejemplos: "Tarifa: 1C", "TARIFA 1A", "Tarifa DAC", "Tarifa Doméstica", etc.
  const m1 = t.match(/\bTARIFA\b[:\s-]([A-Z0-9]{1,4}(?:\s[A-Z]{1,3})?)/i);
  if (m1?.[1]) return m1[1].trim();

  // A veces: "DAC" suelto cerca de tarifa
  const m2 = t.match(/\b(DAC)\b/i);
  if (m2?.[1]) return m2[1].toUpperCase();

  // "DOMESTICA" / "DOMÉSTICA" (como texto)
...
/**
 * Detecta periodo de facturación:
 * - "PERIODO: 01 ENE 2026 AL 01 FEB 2026"
 * - "Periodo de consumo 01/01/2026 - 01/02/2026"
 * - "del 01-ENE-2026 al 01-FEB-2026"
 */
...
/**
 * Detecta lecturas (anterior/actual)
 * Considera:
 * - "LECTURA ANTERIOR 12345 LECTURA ACTUAL 12567"
 * - Tablas donde aparecen ambas cerca
 */
function detectReadings(text) {
  ...
  const rePairs = [
    // ANTERIOR ... ACTUAL
    /\bLECTURA\s+ANTERIOR\b[^0-9]([0-9]{1,9}(?:[.,][0-9]{1,3})?)\b[\s\S]{0,120}?\bLECTURA\s+ACTUAL\b[^0-9]([0-9]{1,9}(?:[.,][0-9]{1,3})?)/i,
    // ACTUAL ... ANTERIOR (por si viene invertido)
    /\bLECTURA\s+ACTUAL\b[^0-9]([0-9]{1,9}(?:[.,][0-9]{1,3})?)\b[\s\S]{0,120}?\bLECTURA\s+ANTERIOR\b[^0-9]([0-9]{1,9}(?:[.,][0-9]{1,3})?)/i,
  ];
  ...
  for (const re of rePairs) {
    ...
    if (m?.[1] && m?.[2]) {
      let prev = null;
      let curr = null;
      if (/ANTERIOR/i.test(re.source) && /ACTUAL/i.test(re.source)) {
        // depende del orden del regex
      }

      // Determinar quién es quién por texto del match:
      // Si el regex es ANTERIOR ... ACTUAL => m1 prev, m2 curr
      if (re === rePairs[0]) {
        prev = toNumberFlex(m[1]);
        curr = toNumberFlex(m[2]);
      } else {
        // ACTUAL ... ANTERIOR => m1 curr, m2 prev
        curr = toNumberFlex(m[1]);
        prev = toNumberFlex(m[2]);
      }

      if (prev != null && curr != null) {
        candidates.push({ prev, curr, score: 5 });
      }
    }
  }

  // Fallback: capturar ocurrencias sueltas
  ...
  // Elegir el más razonable: curr >= prev
...
/**
 * Detecta kWh:
 * - Preferir líneas con "kWh" + "Consumo" / "Energía"
 * - Si hay varios números, elegir el que parece consumo
 */
function detectKwh(text) {
  const t = normalizeText(text);
  const lines = t.split("\n").flatMap((ln) => ln.split(/(?<=kWh|KWH|kwh)\s+/)).map(s => s.trim()).filter(Boolean);
  ...
  // Fallback: número antes de kWh global
  ...
  // Elegimos el primer número "preferido"
  ...
  return {
    kwh,
    confidence,
    candidates: candidates.slice(0, 10).map((c) => • ${c.line}),
  };
}

/**
 * Fusiona señales: preferir kWh explícito; si no, usar (curr-prev) si existe y es positivo.
 */
function computeFinalSignals({ textMain, textOcr }) {
  ...
  // kWh explícito: preferir el de mejor confianza
  let chosenKwh = null;
  let confidence = 0;
  let method = "—";
  ...
  // Decisión:
  // 1) Si hay kWh explícito confiable, úsalo.
  // 2) Si no hay, usa diff por lecturas (si existe).
  // 3) Si ambos existen pero difieren mucho, muestra el explícito y baja confianza.
  if (bestKwhSignal && bestKwhSignal.kwh != null) {
    ...
    if (diffKwh != null) {
      ...
      // si se va demasiado (por ejemplo > 10%), reduce confianza y deja nota
      if (chosenKwh > 0 && delta / chosenKwh > 0.12) {
    ...
    chosenKwh = diffKwh;
    confidence = 0.55; // moderada: depende de lectura/cambio de medidor, etc.
...
/**
 * Heurística: si el texto extraído es muy corto, probablemente es escaneado => OCR recomendado.
 */
function needsOcr(text) {
  const t = normalizeText(text);
  // si hay muy pocos caracteres o casi no hay letras
...
async function analyze({ forceOcr = false } = {}) {
  ...
  try {
    ...
    const { pdf, text } = await loadPdf(currentFile);
    currentPdf = pdf;
    extractedText = text;
    ...
    // Si parece escaneado o el usuario lo forzó, hacer OCR
...
els.pdfInput.addEventListener("change", (e) => {
  ...
  if (!file) return;
  if (!isPdf(file)) return setStatus("Eso no es un PDF", "bad");
  bindFile(file);
});
